...
$annTable = "announcements_temp2";
//Get all annNumbers from the database
$sqlGlob = "SELECT url FROM urls LIMIT 21000,1000;";
...
while ($row = $allUrls->fetch(PDO::FETCH_ASSOC)):
      ...
      if($resAddAnn):
         $sql2 = "INSERT INTO ".$annImagesTable."(
                           annID,
                           imageName) VALUES";
         //Запись картинок
         if($data[$annNumber]):
            foreach($imageUrl[$annNumber] as $key1 => $url):
               ...
               file_put_contents($imagePath . substr($imageN,0,2) ."/". array_pop(explode('/', $url)) , file_get_contents($url));
               unset($imageN);
               $sql2 .="(
                       '" . $annNumber . "',

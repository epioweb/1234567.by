...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(IntVal($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		{
            $arUserResult["USER_EMAIL"] = randString(8)."@".$_SERVER["SERVER_NAME"];
			if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
			{
				if(strlen($arUserResult["USER_EMAIL"]) > 0)
				{
			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				if (empty($arResult["ERROR"]))
				{
                    ...
                    // добавляем в массив значение свойств
                    foreach($arOrderProps as $arProp)
                        $arFields[$arProp["CODE"]] = $arProp["VALUE"];

                    // инфо о выбранной службе доставки
                    ...
                    $arFields["COMMENT"] = $arUserResult["ORDER_DESCRIPTION"];

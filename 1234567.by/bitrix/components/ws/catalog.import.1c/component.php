...
else
{
	...
	{
       // $_SESSION["BX_CML2_IMPORT"]["NS"]["STEP"] = 4;
		...
		elseif($NS["STEP"] == 4)
		{
			$obCatalog = new CExIBlockCMLImport;
		...
		{
            //$obCatalog = new CExIBlockCMLImport;
            //$obCatalog->RecalcSectionQuantity(0);
            /*AddMessage2Log("финальный шаг. Сброс кеша и пересчет счетчиков");
            // инициируем общий кеша
            if(CModule::IncludeModule("nscache"))
            {
                $exChache = new CExCacheM;
                $exChache->CleanCommon();
            }*/
            ...
            if(CModule::IncludeModule("nscache"))
            {
                //add2Log("запущен общий сброс кеша ". date("Y-m-d H:s:i", time()));
                $exChache = new CExCacheM;
                $exChache->CleanCommon();
            }
    ...
    elseif($_GET["mode"] == "recalc")
    {
        //пересчет счетчиков
        $obCatalog = new CExIBlockCMLImport;
        $obCatalog->RecalcSectionQuantity(0);
        $obCatalog->UpdateStatusState();

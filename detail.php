...
$annNumber=$_REQUEST["annNumber"];


$sql1 = "SELECT * FROM ".$section."_".$annTable." WHERE annNumber=". $annNumber .";";
$res1 = $db->query($sql1) or die(mysql_error());

$announce = $res1->fetch(PDO::FETCH_ASSOC);
if(!$announce):
   header('Location: /errors/error-404.html');
endif;
 
$sql2 = "SELECT * FROM  ".$section."_annImages WHERE annID=". $annNumber .";";
   ...
   <div id="ann_detail_column">  
      <div class="ann_item">
         ...
         <div class="ann_photos">
            <center>
         <?$i=0; while ($images = $res2->fetch(PDO::FETCH_ASSOC)):?>
            <img class="ann_detail_photo <?=($i !=2) ? 'center' : ''?>" src="/<?=$imagePath.$section?>/<?=substr($images['imageName'],0,2)?>/<?=$images['imageName']?>"/>
         <? $i++;
            if($i == 2): break; endif;
            endwhile; ?>
            </center>
      ...
      <?
      //формированик данных для "кольца"
      $sql3 = "SELECT * FROM ".$section."_".$annTable." WHERE annNumber>". $announce['annNumber'] ." ORDER BY annNumber ASC LIMIT 0,". $similar .";";
      $res3= $db->query($sql3);
      while($anns = $res3->fetch(PDO::FETCH_ASSOC)):
      	$arAnns[] = $anns;
      endwhile;
      //shuffle($arAnns);
      $additionals= $similar-$res3->rowCount(); //количетво объявлений, выводимых с начала
      if($additionals !=0):
         $sql3 = "SELECT * FROM ".$section."_".$annTable." ORDER BY annNumber ASC LIMIT 0,". $additionals .";";
         $res3= $db->query($sql3);
         while($anns = $res3->fetch(PDO::FETCH_ASSOC)):
      	    $arAnns[] = $anns;
         endwhile;
      ...
      $sql4 = "SELECT * FROM ". $section ."_". $annImagesTable ." WHERE annID IN(";
      $cn=0;
      ...
      while($ann2= $res4->fetch(PDO::FETCH_ASSOC)):
      ...
      <div style="margin: 0 5;">
         ...
         <div style="float:left;">
            ...
            <div style="width:90px; margin:0 10; float:left;">
               <a href="/<?=$section?>/detail/<?=$keyAnnID;?>.html">
                  <img style="height:90px !important; width:90px !important;" src="/<?=$imagePath?><?=$section?>/<?=(isset($image["images"][0])) ? (substr($image["images"][0],0,2)."/".$image["images"][0]) : 'no_photo.jpg'?>"/>
               <div><?=$image["title"]?></div>
               </a>
               <div><?=$image["price"]?></div>
            </div>
